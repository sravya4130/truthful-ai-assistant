...
serve(async (req) => {
  ...
  try {
    ...
    if (!aiResp.ok) {
      ...
      return new Response(JSON.stringify({ error: "Image edit failed" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: record, error: insErr } = await userClient
      ...
      .insert({ user_id: user.id, prompt: `[edit] ${prompt}`, image_url: newUrl })
      .select()
      .single();
    if (insErr) console.error("Insert error:", insErr);

    return new Response(JSON.stringify({ image: record ?? { prompt, image_url: newUrl } }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
